...
if run_sim:
    ...
    for i in range(months):
        ...
        if account_type == "Taxable":
            tax_owed = div_income * (fed_tax_rate + state_tax_rate) / 100
            if year not in tax_records:
                tax_records[year] = {"owed": 0.0, "paid": 0.0}
            tax_records[year]["owed"] += tax_owed

        total_penalty = 0
        tax_paid = 0
        total_due_this_month = 0

        for y in sorted(tax_records.keys()):
            due_date = datetime(y + 1, 10, 15)
            if defer_taxes and date >= due_date:
                balance = tax_records[y]["owed"] - tax_records[y]["paid"]
                if balance > 0:
                    fed_penalty = balance * fed_penalty_rate
                    state_penalty = balance * state_penalty_rate
                    penalty = fed_penalty + state_penalty
                    total_due_this_month += penalty + balance
                    total_penalty += penalty
                    pay_now = min(gross_cash - withdrawal - tax_paid, penalty + balance)
                    tax_records[y]["paid"] += pay_now - penalty
                    tax_paid += pay_now

        if not defer_taxes and account_type == "Taxable":
            tax_paid += tax_owed
        ...
        rows.append({
            ...
            "Penalty This Month": round(total_penalty, 2),
