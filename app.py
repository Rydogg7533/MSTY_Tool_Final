...
with tabs[1]:
    ...
    shares = st.number_input("Number of Shares", min_value=1, value=100, key="cb_shares")
    price = st.number_input("Price per Share ($)", min_value=0.01, value=25.00, key="cb_price")
    ...
    if st.session_state["blocks"]:
        ...
        total_shares = df_cb["Shares"].sum()
        total_cost = df_cb["Total Cost"].sum()
        ...
        csv = df_cb.to_csv(index=False).encode("utf-8")
